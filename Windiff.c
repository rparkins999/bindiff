...
CreateTools(void)
{
		...
		OemFixedFont = CreateFont(
			20,					// logical height of font
			0,					// closest match for aspect ratio
...
FindNextChange(void)
{
        ...
        if (row >= 0) {
                ...
                windiff_UI(TRUE);
                MessageBox(hwndClient, LoadRcString(IDS_NO_MORE_CHANGES), "Bindiff",
...
FindPrevChange(void)
{
        ...
        if (row >= 0) {
                ...
                windiff_UI(TRUE);
                MessageBox(hwndClient, LoadRcString(IDS_NO_PREV_CHANGES), "Bindiff",
                        MB_ICONINFORMATION|MB_OK);
                windiff_UI(FALSE);
...
SetNotBusy(void)
{
        ...
        SetWindowText(hwndClient, "BinDiff");
...
do_getprops(HWND hwnd, lpColPropsList propslist)
{
        ...
        for (i = 0; i < propslist->ncols; i++) {
                ...
                if (!bIsPrinter) {
                        ...
                        propslist->plist[i].props.hFont = OemFixedFont;
                }
        }
        return (TRUE);
}
...
MainWndProc(HWND hWnd, UINT message, UINT wParam, LONG lParam)
{
        ...
        case WM_CREATE:
                ...
                CheckMenuItem(hMenu, IDM_RECURSIVE,
                        dlg_recursive ? MF_CHECKED : MF_UNCHECKED);
        ...
        case WM_COMMAND:
                ...
                case IDM_DIR:
                        ...
                        break;
				case IDM_RECURSIVE:
						/* flip state of recursive directory expansion switch */
						dlg_recursive = !dlg_recursive;
						CheckMenuItem(hMenu, IDM_RECURSIVE,
								dlg_recursive? MF_CHECKED:MF_UNCHECKED);
						break;
