...
complist_dirdialog(VIEW view)
{
    ...
    if (SHGetMalloc(&pMalloc) == NOERROR)
	{
        ...
        /* allow user to browse for two directories */

		/* prepare BROWSEINFO structure */
		bif.hwndOwner = hwndClient;
		bif.pidlRoot = NULL;
		bif.pszDisplayName = dialog_leftname;
		bif.lpszTitle = "Select First Directory";
		bif.ulFlags = BIF_RETURNFSANCESTORS | BIF_EDITBOX | BIF_RETURNONLYFSDIRS;
		bif.lpfn = NULL;

        windiff_UI(TRUE);
        /* This next call issues the dialog box. */
        if ((pidl = SHBrowseForFolder(&bif)) != NULL)
		{
            if (!SHGetPathFromIDList(pidl, dialog_leftname))
			{ 
                return NULL;
			}
		...
		bif.hwndOwner = hwndClient;
		bif.pidlRoot = NULL;
		bif.pszDisplayName = dialog_rightname;
		bif.lpszTitle = "Select Second Directory";
		bif.ulFlags = BIF_RETURNFSANCESTORS | BIF_EDITBOX | BIF_RETURNONLYFSDIRS;
		bif.lpfn = NULL;
        ...
        if ((pidl = SHBrowseForFolder(&bif)) != NULL)
		{
            if (!SHGetPathFromIDList(pidl, dialog_rightname))
			{ 
                return NULL;
			}
            pMalloc->lpVtbl->Free(pMalloc, pidl);
...
complist_savelist(COMPLIST cl, LPSTR savename, UINT options)
{
        ...
        if (fh < 0) {
                ...
                windiff_UI(TRUE);
                MessageBox(NULL, msg, "Bindiff", MB_ICONSTOP|MB_OK);
                windiff_UI(FALSE);
                return;
        }
...
complist_copyfiles(COMPLIST cl, LPSTR newroot, UINT options)
{
        ...
        windiff_UI(TRUE);
        MessageBox(hwndClient, buffer, "BinDiff", MB_OK|MB_ICONINFORMATION);
        windiff_UI(FALSE);
