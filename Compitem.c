...
/***************************************************************************
 * Function: compare_quick
 *
 * Purpose:
 *
 * Does a quick comparison of the two files to see if they differ
 *
 * Returns nonzero if so
 *
 ****************************************************************************/
#define BUFFLENGTH 512
static int compare_quick (COMPITEM ci)
{
	int result = -1;
	int fhl, fhr;
	char lbuff [BUFFLENGTH];
    ...
    /* open the files  */
    fhl = dir_openfile(file_getdiritem(ci->left));
    /* open the file */
    fhr = dir_openfile(file_getdiritem(ci->right));

	while (result < 0)
	{
		(void)ReadFile((HANDLE)fhl, lbuff, BUFFLENGTH, &lbytes, NULL);
		(void)ReadFile((HANDLE)fhr, rbuff, BUFFLENGTH, &rbytes, NULL);

		if ((lbytes == 0) && (rbytes == 0))
		{
			result = 0;
		}
		else if (lbytes != rbytes)
		{
			result = 1;
		}
		else
...
compitem_new(DIRITEM leftname, DIRITEM rightname, LIST list, BOOL fExact)
{
        ...
        if (ci->left == NULL) {
                ...
                } else if (compare_quick (ci)) {
                    ci->result =  ci_copytext(LoadRcString(IDS_FILES_DIFFER));
                    ci->state = STATE_DIFFER;
                } else {
